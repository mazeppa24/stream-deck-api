...
const FONT_FILE = '/../fonts/ds_digital/DS-DIGI.ttf';
...
class StreamDeck extends EventEmitter {
    ...
    drawText(text, textColor, buttonColor, buttonNumber) {
        ...
        var buffer = text2png(text, {
            font: fontSize + 'px DS-Digital',
            padding: padding,
            localFontName: 'DS-Digital',
            localFontPath: glob.sync(path.join(__dirname, FONT_FILE)),
            color: textColor,
     ...
     * @returns {Promise<any>}
     */
    drawTextOverImage(text, textColor, imageFile, buttonNumber) {
        return new Promise((resolve, reject) => {
            Jimp.read(imageFile, (error, image) => {
                if (error) {
                    logger.error('Could not find file: ' + imageFile, error);
                } else {
                    return new Promise((resolve, reject) => {
                        Jimp.loadFont(Jimp.FONT_SANS_64_WHITE, (error, font) => {
                            if (error) {
                                console.log('Could not load font', error);
                            } else {
                                //image.print(font, 50, 120, 'TEST');
                                image.print(font, 10, 120, text);
                                image.contain(ICON_SIZE, ICON_SIZE);
                                //image.write('resources/images/ldg_taxi_light_off.png');
                                this.drawImageBuffer(image.bitmap.data, buttonNumber);
                                //this.drawImageFile('resources/images/ldg_taxi_light_off.png', 1);
                                resolve(font);
                            }
                        });
                    });
