...
class StreamDeck extends EventEmitter {
  ...
  /**
   * Draw a given string to a button
   * @param text
   * @param color
   * @param buttonNumber
   * @returns {Promise<any>}
   */
  drawText(text, color, buttonBckgrndColor, buttonNumber, px) {

    var buffer = text2png(text, {
      font: px+ 'px DS-Digital',
      textAlign: 'center',
      padding: 10,
      localFontName: 'DS-Digital',
      localFontPath: './fonts/ds_digital/DS-DIGI.ttf',
      color: color,
      backgroundColor: buttonBckgrndColor,
      output: 'buffer'
    });
  ...
  drawTextFile(text, buttonNumber) {
    return new Promise((resolve, reject) => {
      Jimp.read("./images/blank.png", (error, image) => {
        if (error) {
          console.log('ERROR');
        } else {
          return new Promise((resolve, reject) => {
